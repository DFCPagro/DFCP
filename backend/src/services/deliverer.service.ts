...
export const createDeliverer = async (
  payload: Partial<InstanceType<typeof Deliverer>>
) => {
  const doc = await Deliverer.create(payload as any);
  return doc;
};

export const getDelivererById = async (
  id: string,
...
export const listDeliverers = async (params: ListDeliverersParams = {}) => {
  ...
  if (typeof hasVehicleInsurance === "boolean")
    filter.vehicleInsurance = hasVehicleInsurance;
...
export const updateDeliverer = async (
  ...
  const doc = await Deliverer.findByIdAndUpdate(id, update, {
    new: true,
    runValidators: true,
    ...options,
  });
...
export const listCentersForDeliverer = async (delivererId: string) => {
  const d = await Deliverer.findById(delivererId, {
    logisticCenterIds: 1,
    user: 1,
  }).lean();
  if (!d) throw new ApiError(404, "Deliverer not found");
  const centers = await LogisticsCenter.find({
    _id: { $in: d.logisticCenterIds },
  }).lean();
  return centers;
};

/** Assign deliverer to a center and ensure LC.employeeIds includes the deliverer.user */
export const assignDelivererToCenter = async (
  delivererId: string,
  centerId: string
) => {
  if (
    !mongoose.isValidObjectId(delivererId) ||
    !mongoose.isValidObjectId(centerId)
  ) {
...
export const unassignDelivererFromCenter = async (
  delivererId: string,
  centerId: string
) => {
  if (
...
export const listDeliverersByCenter = async (
  centerId: string,
  ...
  const filter = {
    logisticCenterIds: { $in: [new mongoose.Types.ObjectId(centerId)] },
  };
  const [items, total] = await Promise.all([
    Deliverer.find(filter)
      .sort(sort)
