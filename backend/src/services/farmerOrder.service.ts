...
export async function patchContainerWeightsService(args: {
  ...
  const normalized = (weights || [])
    ...
    .filter((w) => w.containerId && Number.isFinite(w.weightKg) && w.weightKg >= 0)
  ...
  if (!opsDocs.length) {
    throw new ApiError(400, "No matching containerIds found for this FarmerOrder")
  }
  ...
  const snapshotIndex = new Map(
    (fo.containerSnapshots as any[]).map((s: any, idx: number) => [s.containerId, idx] as const)
  )
  ...
  fo.updatedAt = now
  fo.addAudit(user.userId, "CONTAINER_WEIGHTS_PATCH", `Updated ${updatedCount} container weights`, {
