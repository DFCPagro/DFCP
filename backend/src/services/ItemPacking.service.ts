...
// Stateless packing engine — now aware of ItemPacking overrides and "use bigger box to reduce box count" heuristic.
...
export type ItemPackingOverride = {
  fragility?: Fragility; // "very_fragile" | "fragile" | "normal" | "sturdy"
  allowMixing?: boolean;
  requiresVentedBox?: boolean;
  minBoxType?: "Small" | "Medium" | "Large";
  maxWeightPerPackageKg?: number; // per box, for this item
  maxKgPerBag?: number; // split bags by this instead of fragility cap
  densityKgPerL?: number; // override density
  unitVolLiters?: number; // fallback liters per unit if avgWeightPerUnitGr unknown
};
...
export type PackageSizeLite = {
  ...
  headroomPct?: number; // if not provided, DEFAULT_HEADROOM used
  usableLiters?: number; // if not provided, computed from dims+headroomPct
  maxWeightKg: number;
  vented?: boolean;
  // extra helpful constraints
  maxSkusPerBox?: number; // how many distinct SKUs allowed to mix
...
// default kg-per-bag by fragility
...
// Bigger-box heuristic: if a piece would fill >= this % of the smallest feasible box,
// and a larger box is available, we escalate to the next size to reduce box count.
const ESCALATE_WHEN_FILL_PCT_GE = 0.7;

// Optional: also consider expected liters for the *same item* still unplaced.
// If (current piece liters + next-same-item liters) fits in the next size, prefer the larger box.
...
// treat common SKUs as pre-bundled by name/type
function treatAsBundle(
  item: ItemLite
):
  | { enabled: boolean; sizeUnits?: number; perBundleLiters?: number }
  | undefined {
  const t = (item.type || "").toLowerCase();
  if (t.includes("egg"))
...
function buildPiecesForLine(
  ...
  // Bundles (eggs/bread/milk) — strong items preserved as units
  ...
  if (bundle?.enabled && mode === "unit" && (line.units ?? 0) > 0) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      out.push({
        ...
        units: u,
        liters: round2(
          bundle.perBundleLiters ?? litersFromUnits(item, u, overrides)
        ),
        fragility: f,
        allowMixing: true,
        requiresVented: vent,
  ...
  // Bags (default): split by kg cap (either override or by fragility)
  if (mode === "kg") {
    ...
    const perUnitKg = item.avgWeightPerUnitGr
      ? item.avgWeightPerUnitGr / 1000
      : 0.15;
    ...
    while (rem > 0) {
      ...
      out.push({
        ...
        units: bagU,
        liters: round2(
          litersFromUnits(item, bagU, overrides) + OVERHEAD_L_PER_BAG
        ),
        fragility: f,
        allowMixing: mix,
        requiresVented: vent,
...
function sortPiecesForPlacement(pieces: Piece[]): Piece[] {
  ...
  return [...pieces].sort((a, b) => {
    ...
    return b.liters - a.liters; // bigger bases first
  });
}
...
function autoSplitIfNeeded(piece: Piece, sizes: PackageSizeLite[]): Piece[] {
  ...
  // split in halves until it fits (guarded)
  const out: Piece[] = [];
  const q: Piece[] = [piece];
  let guard = 0;
...
function canPlaceIn(box: OpenBox, piece: Piece): boolean {
  ...
  // global box mixing rule
  if (box.type.mixingAllowed === false && box.contents.length > 0) {
    // if mixing not allowed, only same item allowed
    if (!box.contents.every((c) => c.itemId === piece.itemId)) return false;
  }

  // minimal item mixing rule: very_fragile not mixed by default (allowMixing=false)
  if (piece.allowMixing === false) {
    const distinct = new Set(box.contents.map((c) => c.itemId));
    if (distinct.size > 0 && (!distinct.has(piece.itemId) || distinct.size > 1))
      return false;
  }

  // per-item kg cap inside a box (override)
...
/**
 * If we are about to open a new box for this piece, decide whether to escalate to a bigger box
 * to reduce total box count:
 *  - If projected fill of the smallest feasible box >= ESCALATE_WHEN_FILL_PCT_GE, and a larger box exists → escalate.
 *  - Or if (this piece + a few next same-item pieces) can fit in the next size → escalate.
 */
function maybeEscalateBoxChoice(
  ...
  // Rule 1: near-full smallest → escalate
  if (fillPct >= ESCALATE_WHEN_FILL_PCT_GE) return next;

  // Rule 2: lookahead for same item to coalesce
  const same = remainingPieces
...
/**
 * Compute a packing plan for explicit lines.
 * @param lines           items with kg or units
 * @param itemsById       item data (must include _id, name, avgWeightPerUnitGr when possible)
 * @param itemPackingById optional overrides per item (ItemPacking)
 * @param packageSizes    available package sizes
 */
export function computePackingPlan(
  lines: OrderLineLite[],
  itemsById: Record<string, ItemLite>,
  ...
  for (const line of lines) {
    ...
    for (const p of built) {
      ...
      if (split.length > 1)
        warnings.push(`Auto-split piece of item ${p.itemId} to fit boxes.`);
      pieces.push(...split);
    }
  }
  ...
  for (let i = 0; i < pieces.length; i++) {
    ...
    // try place into existing boxes first
    let placed = false;
    ...
    // open a new box — choose smallest feasible, maybe escalate to reduce box count
    ...
    const chosen = maybeEscalateBoxChoice(
      piece,
      smallest,
      packageSizes,
      remaining
...
/** ---- Overloads: computePackingForOrderDoc ---- */
...
/**
 * Convenience for your order docs shaped as: { items: [{ itemId, quantity? | quantityKg? | units? }] }
 * — now accepts optional ItemPacking map.
 */
export function computePackingForOrderDoc(
  order: {
    items: Array<{
  ...
  return computePackingPlan(lines, itemsById, packageSizes, itemPackingById);
}
