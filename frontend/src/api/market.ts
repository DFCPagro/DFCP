...
const ItemRaw = z.object({
  _id: z.string().optional(), // subdoc id
  itemId: z.string(),
...
export async function getAvailableShiftsByLC(
  LCid: string
  ...
  const raw = (Array.isArray(data?.data) ? data.data : data) as
    | any[]
    | undefined;
...
export async function getStockByMarketStockId(
  marketStockId: string
): Promise<MarketStockDoc> {
  try {
    const url = `/market/available-stock/${encodeURIComponent(marketStockId)}`;
    ...
    // Parse the raw payload with Zod
    let raw: any;
    try {
      raw = DocRaw.parse(payload);
    } catch (zerr) {
      console.error("[getStockByMarketStockId] DocRaw.parse failed", {
        error: zerr,
        payload,
      });
      throw zerr;
    ...
    // Defensive array guard (warn only when shape is off)
    const items = Array.isArray(raw.items) ? raw.items : [];
    if (!Array.isArray(raw.items)) {
      console.warn("[getStockByMarketStockId] 'raw.items' is not an array", {
        value: raw.items,
      });
    }
    ...
    const normalized: MarketStockDoc = {
      ...
      logisticCenterId:
        raw.LCid ?? raw.logisticsCenterId ?? raw.logisticCenterId,
      lines: items.map((x: any) => {
        ...
        const avgWeightPerUnitKg =
          typeof avgW === "number" && Number.isFinite(avgW) && avgW > 0
            ? avgW
            : 1;
        ...
        if (
          (availableUnitsEstimate == null ||
            Number.isNaN(availableUnitsEstimate)) &&
          ...
          availableUnitsEstimate = Math.floor(
            x.currentAvailableQuantityKg / avgWeightPerUnitKg
          );
        }
        ...
        return {
          ...
          imageUrl: x.imageUrl,
          category:
            (x.category ?? "misc").toString().trim().toLowerCase() || "misc",
          ...
          estimates: {
            avgWeightPerUnitKg:
              x.estimates?.avgWeightPerUnitKg ?? avgWeightPerUnitKg ?? null,
    ...
    try {
      ...
      console.error(
        "[getStockByMarketStockId] MarketStockDocSchema.parse failed",
        { error: zerr, normalized }
      );
      throw zerr;
    }
  } catch (err) {
    console.error("[getStockByMarketStockId] Unhandled error", err);
    throw err;
  }
}
...
export async function getFlatItemsByMarketStockId(
  marketStockId: string
): Promise<MarketItem[]> {
  // console.log(
  //   "Fetching and flattening items for marketStockId:",
  //   marketStockId
  // );
  const doc = await getStockByMarketStockId(marketStockId);
  // console.log("Fetched MarketStockDoc:", doc);
...
export async function getMarketItemPage(
  itemId: string,
  farmerUserId: string
): Promise<MarketItemPageData> {
  const { data } = await api.get(
    `items/marketItemPage/${encodeURIComponent(itemId)}/${encodeURIComponent(farmerUserId)}`
  );
