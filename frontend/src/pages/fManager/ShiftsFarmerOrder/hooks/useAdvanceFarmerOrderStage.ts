  ...
  return useMutation({
    ...
    onMutate: async (vars) => {
      ...
      await qc.cancelQueries({
        predicate: ({ queryKey }) =>
          Array.isArray(queryKey) && queryKey[0] === "farmer-orders-by-shift",
      });
    ...
    onSuccess: (updatedItem) => {
      const updater = (old?: FarmerOrdersByShiftCache) => {
        if (!old) return old;
        const items = old.items?.map((it) =>
          (it as any)?._id === (updatedItem as any)?._id ? updatedItem : it
        );
        return { ...old, items };
      };
    ...
    // at the end of the mutation options:
    onSettled: () => {
      qc.invalidateQueries({
        predicate: ({ queryKey }) =>
          Array.isArray(queryKey) && queryKey.join("|").includes("by-shift"),
      });
    },
  });
}
